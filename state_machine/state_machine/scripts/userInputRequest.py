...
class UserInputRequest(smach.State):
    ...
    def __init__(self, action='user_request'):
        ...
        self.action = action
        if self.action == 'user_request': srv_name = '/seqslam_tpp/user_input_request'
    ...
    def execute(self, userdata):
        ...
        try:
            response = self.service(UserSelectionRequest())

            if response.success.data:

                userdata['data']['initial_image'] = response.image
                userdata['data']['user_selection'] = response.selection
