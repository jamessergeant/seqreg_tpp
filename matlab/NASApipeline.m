...
%%
    properties
        ...
        fixedPoints
        movingPoints
        search_fract = 0.6; % region of image to use
%             search_fract = 0.9;
        xres
        yres
...
%         border = 0;   % ignore border cases
        ...
        gifdir = '/home/james/datasets/pixl-watson/gifs/'
        test_corr
        test_corr_nss
        results = {}
        test_totals
...
%%
        ...
        function res = seqslamCallback(obj,~,req,res)
            ...
            fprintf('Initial Registration\n\tEstimated Scale:\t%0.3f\n\tEstimated Rotation:\t%0.3f\n\tTrans X: %0.3f\n\tTrans Y: %0.3f\n',scaleRecovered,thetaRecovered,obj.tform.T(3,2),obj.tform.T(3,1));
            
            if abs(1 - scaleRecovered) < 2.0 && abs(thetaRecovered) < 10
...
%             fprintf('Method Time: %0.4f seconds\n',t_total)
            ...
            if obj.save_images && obj.test_cases(obj.curr_case).(obj.method).match
                obj.save_im();
...
%%
        ...
        % generate gif using the estimated transform
            ...
            % transform
            if ~obj.test_cases(obj.curr_case).(obj.method).match
                return
            end
            ...
            % should update this to make consistent
            if ~strcmp(obj.method,'cnn')
                [xLimitsOut,yLimitsOut] = outputLimits(obj.test_cases(obj.curr_case).(obj.method).tform, [1 size(im2f,2)], [1 size(im2f,1)]);
                ...
                [xLimitsOut,yLimitsOut] = outputLimits(obj.test_cases(obj.curr_case).(obj.method).tform, [1 size(im1f,2)], [1 size(im1f,1)]);
            ...
            % apply estimated transform to im1
            registered = imwarp(im1f, obj.test_cases(obj.curr_case).(obj.method).tform, 'OutputView', outView);
            ...
            framename = sprintf('%s%s.gif',obj.gifdir,outstring);
...
%%
        ...
        % generate the results html page showing all cases and results
            f = fopen(sprintf('%sweb/ind_results.html',obj.save_dir),'w');
            ...
            for i = 1:length(obj.test_cases)
                ...
                if any(obj.known_bad_cases == i)
                    ...
                    fprintf(f,sprintf(string, ...
                        ...
                        bool2mark(obj.test_cases(i).seqslam.match), ...
...
%%
        ...
        % summarised results page
            ...
            CNN = obj.results.(obj.method);
            CNN = CNN(ind);
            obj.results.(obj.method) = CNN;

            obj.summ_testing('surf');
            assert(all(Total(:) == obj.test_totals(:)));
            SURF = obj.results.(obj.method);

            obj.summ_testing('seqslam');
            assert(all(Total(:) == obj.test_totals(:)));
            SeqSLAM = obj.results.(obj.method);
        ...
        % find a random example where all methods were successful
            ...
            while ~all(status)
                ...
                for i = 1:3
                    ...
                    if obj.test_cases(obj.curr_case).(obj.method).scaleRecovered > 2
                        continue
                    elseif obj.test_cases(obj.curr_case).(obj.method).scaleRecovered < 0.5
                        continue
                    elseif abs(obj.test_cases(obj.curr_case).(obj.method).thetaRecovered) > 7.5
                        continue
                    end
                end
...
%%
        ...
        % perform seqslam image registration
            ...
            corr_rangex = round(obj.search_fract * obj.xres);
            corr_rangey = corr_rangex; % mightn't this exceed image dimensions?
            
%             black_dim1=floor(sum(all(obj.im1 == 0,1))/2);
%             black_dim2=floor(sum(all(obj.im1 == 0,2))/2);
%             
%             for_vis1 = ~all(obj.im1 == 0,2);
%             for_vis2 = ~all(obj.im1 == 0,1);
%             
%              obj.im1 = obj.im1(~all(obj.im1 == 0,2),~all(obj.im1 == 0,1));
            ...
            if obj.visuals
                ...
                subplot(1, 2, 1);
                imshow(obj.im1_orig);
%                 imshow(obj.im1_orig(for_vis1,for_vis2,:));
                caxis([-2 2]);
                axis on;
                subplot(1, 2, 2);
                imshow(obj.im2_orig);
            ...
            % Search start points
            xrange = (round(obj.border * obj.xres):obj.step_size:round( (1-obj.border) * obj.xres) - obj.seq_len);
            yrange = (round(obj.border * obj.yres):obj.step_size:round( (1-obj.border) * obj.yres) - obj.seq_len);
%             xrange = (round(obj.border * obj.xres):obj.step_size:round( (1-obj.border) * obj.xres));
%             yrange = (round(obj.border * obj.yres):obj.step_size:round( (1-obj.border) * obj.yres));
            ...
            if obj.gpu_processing || obj.gpu_devel_mode
                ...
                im2_regions = zeros([obj.ps*2+1,(obj.ps*2+1)*obj.num_steps],'single','gpuArray');
                im2_region_mean = zeros(obj.num_steps,'single','gpuArray');
                im2_region_std = zeros(obj.num_steps,'single','gpuArray');
                im1_padded = gpuArray(single(padarray(obj.im1,[obj.ps obj.ps])));
                im_out = zeros([imsz(1)+2*obj.ps, (imsz(2)+2*obj.ps)*obj.num_steps],'single','gpuArray');
                im_out_sized = zeros([imsz(1)+2*obj.ps, imsz(2)+2*obj.ps],'single','gpuArray');
            ...
            for startx = xrange
                ...
                for starty = yrange
                    % change between horizontal/diagonal trajectory
                    if (0)
                        % Horizontal sequences
                        xp1 = round(startx);
                        yp1 = round(starty);
                        xp2 = round(startx + obj.seq_len);
                        yp2 = round(starty);
                    else
                        % Diagonal sequences
                        xp1 = round(startx);
                        yp1 = round(starty);
                        xp2 = round(startx + obj.seq_len);
                        yp2 = round(starty + obj.seq_len);
                    end

%                     % Need to threshold for boundary cases
%                     xp1 = max(xp1, obj.ps + 1);
%                     yp1 = max(yp1, obj.ps + 1);
%                     xp2 = min(xp2, obj.xres - obj.ps);
%                     yp2 = min(yp2, obj.yres - obj.ps);
...
%                     ppos = ppos(~(any(ppos' <= obj.ps)' | (ppos(:,1)' > (imsz(2) - obj.ps))' | (ppos(:,2)' > (imsz(1) - obj.ps))'),:);
...
%                     %tic
                    ...
                    if obj.gpu_devel_mode
                        ...
                        im2_regions = zeros(obj.ps*2+1,(obj.ps*2+1)*size(ppos,1));
                        ...
                        for z = 1:size(ppos,1)
                            ...
                            p2 = obj.im2(ppos(z, 2)-obj.ps:ppos(z, 2)+obj.ps, ppos(z, 1)-obj.ps:ppos(z, 1)+obj.ps);
                            ...
                            im2_regions(:,(z-1)*(obj.ps*2+1)+1:z*(obj.ps*2+1)) = single(p2);
                            
                        end
                        
                        height = imsz(1) + 2*obj.ps;
                        width = imsz(2) + 2*obj.ps;
                        
                        obj.gpu_normxcorr2.GridSize = [ceil(height / obj.block_size_num_steps),ceil(width / obj.block_size_num_steps)];
                        
                        im_out = feval(obj.gpu_normxcorr2,im1_padded,im_out,im2_regions,im2_region_mean,im2_region_std,width,height,obj.ps);
                        
                        for i=1:obj.num_steps
                            xchist_gpu(:,(i-1)*imsz(2)+1:i*imsz(2)) = im_out(obj.ps+1:height-obj.ps,(i-1)*width+obj.ps+1:i*width-obj.ps);
                        end
                    
                    elseif obj.gpu_processing % SAD implementation
                        ...
                        im_out_sized = feval(obj.gpu_sad2,im1_padded,im_out_sized,im2_regions,offsets(:),width,height,obj.ps);
                        wait(obj.GPU)
                        
                        xchist = gather(im_out_sized(obj.ps+1:height-obj.ps,obj.ps+1:width-obj.ps));
                        xchist = abs(xchist(:) - max(xchist(:)));
                    else

                        % Create the difference matrices for each step along the trajectory
                        xchist = zeros(imsz(1),imsz(2),obj.num_steps);
                        ...
                        for z = 1:size(ppos,1)
                            ...
                            xc1 = xc1(obj.ps + 1: imsz(1) + obj.ps, obj.ps + 1: imsz(2) + obj.ps);

                            % % New way doing in local region
                            % xcsumrng = (max(1, xp1 - corr_rangex):min(obj.xres, xp1 + corr_rangex) );
                            % ycsumrng = (max(1, yp1 - corr_rangey):min(obj.yres, yp1 + corr_rangey) );
                            % xc_temp = normxcorr2(p2, im1(ycsumrng, xcsumrng));
                            % xc2 = zeros(length(ps + 1: imsz(1) + ps), length(ps + 1: imsz(2) + ps));
                            % xc2([0:size(xc_temp,1) - 1] + ycsumrng(1), [0:size(xc_temp,2) - 1] + xcsumrng(1)) = xc_temp;

                            % save correlation image
                            xchist(:, :, z) = xc1;

                            if obj.gpu_devel_mode
                                xchist_gpu(:,(z-1)*imsz(2)+1:z*imsz(2)) = single(xc1);
                            end
...
%                     
                        ...
                        for z = 1:size(ppos,1)
                            ...
                            x1finish = min(imsz(2) + xoffset, imsz(2));
                            y1start = max(1, yoffset + 1);
                            y1finish = min(imsz(1) + yoffset, imsz(1));

                            x2start = max(1, -xoffset + 1);
                            x2finish = min(imsz(2) - xoffset, imsz(2));
                            y2start = max(1, -yoffset + 1);
                            y2finish = min(imsz(1) - yoffset, imsz(1));
...
%                     fprintf('Matching processing %0.4f\n',t3)
                    ...
                    if obj.visuals
                        ...
                        plot(obj.fixedPoints(lastnumberc:fpsz(1), 1), obj.fixedPoints(lastnumberc:fpsz(1), 2), 'rx', 'MarkerSize', 5, 'LineWidth', 2);
                        plot([obj.fixedPoints(lastnumberc:fpsz(1), 1) obj.fixedPoints(lastnumberc:fpsz(1), 1) + ppos(end,1) - xp1], [obj.fixedPoints(lastnumberc:fpsz(1), 2) obj.fixedPoints(lastnumberc:fpsz(1), 2) + ppos(end,2) - yp1], 'b-', 'MarkerSize', 5, 'LineWidth', 2);
...
%             obj.movingPoints(:,1) = obj.movingPoints(:,1) + black_dim1;
%             obj.movingPoints(:,2) = obj.movingPoints(:,2) + black_dim2;
%             
%             obj.im1 = padarray(obj.im1,[black_dim2,black_dim1]);
...
%%
        ...
        % perform image registration with cnn method
            ...
            % %Attempted blurring of summed feature map
            % im1_conv1 = imgaussfilt(im1_conv1, min(n1));
            % im2_conv1 = imgaussfilt(im2_conv1, min(n1));
...
%%
        ...
        % align images for current case
            ...
            if obj.tform_status == 0
                ...
                ss = Tinv(2,1);
                sc = Tinv(1,1);
                obj.test_cases(obj.curr_case).(obj.method) = struct();
                obj.test_cases(obj.curr_case).(obj.method).scaleRecovered = sqrt(ss*ss + sc*sc);
                obj.test_cases(obj.curr_case).(obj.method).thetaRecovered = atan2(ss,sc)*180/pi;
                obj.test_cases(obj.curr_case).(obj.method).percentPtsUsed = size(inlierPtsOut,1)/size(mp,1);
                obj.test_cases(obj.curr_case).(obj.method).tform = obj.tform;
                obj.test_cases(obj.curr_case).(obj.method).tform_status = obj.tform_status;

                obj.test_match();

                fprintf('Case %i:\n\tMethod: %s\n\tScale: %f\n\tTheta: %f\n\t%s Pts Used: %0.1f%s\n\n',obj.curr_case,obj.method,obj.test_cases(obj.curr_case).(obj.method).scaleRecovered,obj.test_cases(obj.curr_case).(obj.method).thetaRecovered,'%',obj.test_cases(obj.curr_case).(obj.method).percentPtsUsed*100,'%');
            else
                obj.test_cases(obj.curr_case).(obj.method).tform_status = obj.tform_status;
                obj.test_cases(obj.curr_case).(obj.method).match = 0;
...
%%
        ...
        % align images for current case
            ...
            if obj.test_cases(obj.curr_case).(obj.method).tform_status ~= 0
                fprintf('Transform for case %i not found!\n',obj.curr_case)
                return
            elseif obj.test_cases(obj.curr_case).(obj.method).scaleRecovered > 2
            ...
            if ~strcmp(obj.method,'cnn')
                [xLimitsOut,yLimitsOut] = outputLimits(obj.test_cases(obj.curr_case).(obj.method).tform, [1 size(obj.im2_fullres,2)], [1 size(obj.im2_fullres,1)]);
                ...
                registered = imwarp(obj.im1_fullres, obj.test_cases(obj.curr_case).(obj.method).tform, 'OutputView', outView);%imref2d(size(obj.im2_fullres)));
            else
                [xLimitsOut,yLimitsOut] = outputLimits(obj.test_cases(obj.curr_case).(obj.method).tform, [1 size(obj.im2_unpad,2)], [1 size(obj.im2_unpad,1)]);
                ...
                registered = imwarp(obj.im1_unpad, obj.test_cases(obj.curr_case).(obj.method).tform, 'OutputView', outView);%imref2d(size(obj.im2_fullres)));
            end
        ...
        function reprocess_im(obj,varargin)
            ...
            for i = a:c:b
                ...
                for j = 1:size(modes,1)
                    ...
                    if exist(sprintf('/home/james/datasets/pixl-watson/gifs/%s_%i.gif',obj.method,obj.curr_case),'file') == 2
                        break
                    end
...
%%
        function string = header_html(obj)
            ...
            script = '<script> function process() { var url="file:///home/james/Dropbox/NASA/SeqSLAM/web/pages/" + document.getElementById("url").value + ".html"; location.href=url; return false; } </script>';
            string = sprintf('<!DOCTYPE html> <html lang="en"> <head> <meta charset="utf-8"> <title>QUT / NASA Image Registration</title> <link rel="stylesheet" href="%sweb/css/style.css"> <script src="script.js"></script> %s</head> <body><div align="center"><br /><table><tr align="center"><td width="200"><img src="/home/james/Dropbox/NASA/SeqSLAM/acrv.jpg" height="50" /></td><td width="200"><img src="/home/james/Dropbox/NASA/SeqSLAM/qut.png" height="50" /></td><td width="200"><img src="/home/james/Dropbox/NASA/SeqSLAM/nasa-jpl.gif" height="50" /></td></tr></table><br /><h1><a href="%sweb/index.html">QUT / NASA Image Registration</a><br /></h1><form onSubmit="return process();">Go to test case: <input  type="text" name="url" id="url"><input type="submit" value="Go"></form><br />',obj.save_dir,script,obj.save_dir);
...
%%
        function mutual_information(obj)
            ...
            if ~isdir('/home/james/datasets/pixl-watson/gifs/')
                mkdir('/home/james/datasets/pixl-watson/gifs/');
            end

            framename = sprintf('/home/james/datasets/pixl-watson/gifs/%s.gif',outstring);
...
%%
        function generate_home(obj)
            ...
            f = fopen(sprintf('%sweb/index.html',obj.save_dir),'w');
            fprintf(f,obj.header_html());
...
%%
        function generate_html(obj)
            ...
            f = fopen(sprintf('%sweb/pages/%i.html',root,obj.curr_case),'w');
            fprintf(f,obj.header_html());
            ...
            for i = 1:3
                image = sprintf('/home/james/datasets/pixl-watson/gifs/%s_%i.gif',strtrim(modes(i,:)),obj.curr_case);
                if exist(image, 'file') == 2
                    fprintf(f,sprintf('<td align="center"><a href="%s"><img src="%s" width="500" border="5" style="border-color: %s"/></a></td>',image,image,match2colour(obj.test_cases(obj.curr_case).(strtrim(modes(i,:))).match)));
                else
            ...
            fprintf(f,'</tr><tr>');
            for i = 1:3
                if isfield(tc.(strtrim(modes(i,:))),'tform_status') & tc.(strtrim(modes(i,:))).tform_status == 0

                    fprintf(f,sprintf('<td align="center"><ul><li>Scale: %0.3f</li><li>Rotation: %0.3f</li></td>',tc.(strtrim(modes(i,:))).scaleRecovered,tc.(strtrim(modes(i,:))).thetaRecovered));
                else
...
%%
        function find_examples(obj)
            ...
            for i = 1:length(obj.test_cases)
                ...
                if exist(sprintf('/home/james/datasets/pixl-watson/gifs/cnn_%i.gif',i),'file') && ...
                        exist(sprintf('/home/james/datasets/pixl-watson/gifs/surf_%i.gif',i),'file') && ...
                        exist(sprintf('/home/james/datasets/pixl-watson/gifs/seqslam_%i.gif',i),'file')
...
%%
        function [match,message] = test_match(obj)
            ...
            if isfield(obj.test_cases(obj.curr_case).(obj.method), 'tform_status')
            ...
            if isfield(obj.test_cases(obj.curr_case).(obj.method), 'scaleRecovered')

                if obj.test_cases(obj.curr_case).(obj.method).scaleRecovered < 0.5 || obj.test_cases(obj.curr_case).(obj.method).scaleRecovered > 5
            ...
            if isfield(obj.test_cases(obj.curr_case).(obj.method), 'thetaRecovered')

                if abs(obj.test_cases(obj.curr_case).(obj.method).thetaRecovered) > 15
            ...
            if isfield(obj.test_cases(obj.curr_case).(obj.method), 'tform')
                ...
                if abs(obj.test_cases(obj.curr_case).(obj.method).tform.T(3,2) + v + diff_Y*im1sz(1)/2) > obj.trans_limit * imsz(1) || abs(obj.test_cases(obj.curr_case).(obj.method).tform.T(3,1) + u + diff_X*im1sz(2)/2) > obj.trans_limit * imsz(2)
            ...
            obj.test_cases(obj.curr_case).(obj.method).match = match;
            obj.test_cases(obj.curr_case).(obj.method).message = message;
