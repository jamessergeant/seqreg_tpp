...
class pixl {
   ...
   public:
    pixl() : it_(nh_), move_group("manipulator") {
        ...
        servoSub =
            nh_.subscribe("/pixl/servo_result", 10, &pixl::servoCallback, this);
        watsonScale = 1.0;
        pixlScale = 1.0;
        servoing = false;
            ...
            if (initialise) {
                ...
                try {
                    if (!moveTo(pixl_pose, 2, wait_time)) {
                        throw tf::TransformException("Pixl pose reset failed.");
                    }
                } catch (tf::TransformException ex) {
                    ROS_INFO_STREAM(ex.what());
                }
    ...
    void new_pixl() {
        ...
        pixl_pose.pose.orientation =
            tf::createQuaternionMsgFromRollPitchYaw(0.0, M_PI / 2, M_PI / 2);

        ROS_INFO(
            "\n\nNew PIXL initial position\n\tX: %0.3f\n\tY: %0.3f\n\tZ: "
            "%0.3f\n",
            pixl_pose.pose.position.x, pixl_pose.pose.position.y,
            pixl_pose.pose.position.z);
        // reset_pixl = true;
    }
    ...
    void new_watson() {
        ...
        watson_pose.pose.orientation =
            tf::createQuaternionMsgFromRollPitchYaw(0.0, M_PI / 2, M_PI / 2);

        ROS_INFO(
            "\n\nNew Watson initial position\n\tX: %0.3f\n\tY: %0.3f\n\tZ: "
            "%0.3f\n",
            watson_pose.pose.position.x, watson_pose.pose.position.y,
            watson_pose.pose.position.z);
        reset_watson = true;
    }
    ...
    void matlabCallback(const std_msgs::Float32MultiArray msg) {
        ...
        // objectposz = objectposz + (watsonScale - watsonScale*(1 +
        // (mat(0,0)-1)*watsonMult/pixlMult)) * (float(color.cols) /
    ...
    void open_servo() {
        ...
        while (!servo_success) {
            arm_pose = move_group.getCurrentPose();
            // pixlScale = arm_pose.pose.position.z - CAMERA_OFFSET -
            // objectposz;
            // ROS_INFO_STREAM("pixlScale: " << pixlScale);
            // updateSendScale();
            ...
            if (reg_success) {
                ...
                // ROS_INFO_STREAM("obj estimate: " << objectposz);
                // objectposz = objectposz + (watsonScale - watsonScale*(1 +
                // (servo_scale-1)*watsonMult/pixlMult)) * (float(color.cols) /
                // regionOfInterest.cols);
                // ROS_INFO_STREAM("obj estimate updated: " << objectposz);
    ...
    void mouse_click(int event, int x, int y) {
        switch (event) {
            ...
            case cv::EVENT_RBUTTONDOWN: {
                ...
                scalePub.publish(scale);
                pixlScale = move_group.getCurrentPose().pose.position.z -
                            CAMERA_OFFSET - objectposz;
                updateSendScale();
                break;
            }
